$(function () {
    ...
    let onsub = (n) => {
        if (!subscriber[n]) {
            ...
            $('<div id="' + n + '">' + n + '    (watcing topic is : ' + subscriber[n].topic + ")" + '</div>').prependTo('#content').hide().fadeIn(500);
        } else {
            let domid = "#" + n;
            $(domid).fadeIn(500);
        }
    ...
    let offsub = (n) => {
        if (!subscriber[n]) {
            subscriber[n] = Date.now();
            $('<div id="' + n + '">' + n + '    (watcing topic is : ' + subscriber[n].topic + ")" + '</div>').prependTo('#content').hide();
        } else {
            let domid = "#" + n;
            $(domid).hide(500);
        }
